...
export default function Picks() {
    ...
    useEffect(() => {
        async function query() {
            ...
            try {
                const series = [];
                const p = {};
                const data = await db.collection('series-2021').where('round', '==', round).get();
                data.forEach(i => {
                ...
                setSeries(series);
                try {
                    const data = await db.collection('picks-2021').doc(auth.currentUser.uid).get();
                    if (data.data())
    ...
    const submit = async () => {
        try {
            await db.collection('picks-2021').doc(auth.currentUser.uid).set({ picks: picks, name: auth.currentUser.displayName, updated: new Date() }, { merge: true });
            showSavedIcon();
        } catch (e) {
            console.debug(e)
        }
            ...
            return (
                <div className="Container">
                    ...
                    <div className="PicksMenu">
                        <button className={round === 1 ? "Selected" : ""} onClick={() => setRound(1)}>Round 1</button>
                        <button className={round === 2 ? "Selected" : ""} onClick={() => setRound(2)}>Round 2</button>
                        <button className={round === 3 ? "Selected" : ""} onClick={() => setRound(3)}>Conference Finals</button>
                        <button className={round === 4 ? "Selected" : ""} onClick={() => setRound(4)}>NBA Finals</button>
                    </div>
                    <table >
                        <tbody>
                            {series.sort((a, b) => a.teams[0].seed - b.teams[0].seed).sort((a, b) => a.conference.localeCompare(b.conference)).map((data, index) => {
                                return <tr key={index} style={{ height: 50 }} >
                                    <td style={{ border: '1px solid black', textAlign: 'center' }}>{data.teams[0].seed} {data.teams[0].city} {data.teams[0].name}</td>
                                    ...
                                    <td style={{ border: '1px solid black', textAlign: 'center' }}>
                                        <select disabled={data.tipoff.toDate() >= new Date()} value={picks[data.id].name} onChange={(e) => handleTeamChange(e.target.value, data.id)} >
                                            <option value={data.teams[0].name}>{data.teams[0].name}</option>
                                            <option value={data.teams[1].name}>{data.teams[1].name}</option>
                                        </select>
                                    </td>
                                    <td style={{ border: '1px solid black', textAlign: 'center' }}>
                                        <select disabled={data.tipoff.toDate() >= new Date()} value={picks[data.id].games} onChange={(e) => handleGamesChange(e.target.value, data.id)} >
        ...
        case 'score':
            ...
            return (
                <div className="Container">
                                        ...
                                        .map((data, index) => {
                                            ...
                                            return (
                                                <td className={!data.winner ? 'Unknown' : test === 0 ? 'Wrong' : test === 1 ? 'Ok' : 'Correct'} style={{ border: '1px solid black' }} key={index}>
                                                    {account.picks[data.id].name} in {account.picks[data.id].games}
                                                </td>
                                            )
                                        })}
                                </tr>
                    ...
                    {results.map((account, index) => {
                        return <div style={{ textAlign: 'left' }} key={index}>{account.name} last updated: {account.updated.toDate().toString()}</div>
                    })}
                </div>
            );
        default:
