...
export default function Picks() {
    ...
    useEffect(() => {
        async function query() {
            ...
            try {
                const results = [];
                const data = await db.collection('picks').get();
                data.forEach(i => {
                    results.push(i.data())
                })
                setResults(results);
            ...
            try {
                const all = [];
                const data = await db.collection('series').get();
                data.forEach(i => {
    ...
    const submit = async () => {
        try {
            await db.collection('picks').doc(auth.currentUser.uid).set({ picks: picks, name: auth.currentUser.displayName, updated: new Date() }, { merge: true });
            showSavedIcon();
        } catch (e) {
            console.debug(e)
        }
